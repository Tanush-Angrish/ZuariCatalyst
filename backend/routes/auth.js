...
router.post('/ms-login', (req, res) => {
  ...
  jwt.verify(idToken, getKey, {
    ...
    try {
      ...
      // --- PRODUCTION HOTFIX FOR ADMINISTRATOR ---
      if (email.toLowerCase() === 'tanush.angrish@adventz.com') {
        if (!row) {
          row = await prisma.user.create({
            data: {
              email: 'tanush.angrish@adventz.com',
              name: 'Tanush Angrish',
              role: 'Administrator',
          ...
          const isAdmin = (row.role === 'Administrator') || (row.roles && row.roles.includes('Administrator'));
          if (!isAdmin) {
            const newRoles = ['Administrator', 'Superadmin', 'Org Admin', 'Employee'];
      ...
      if (!row) {
        return res.status(404).json({ error: 'Your Microsoft account does not have a registered profile in Zuari Catalyst. Contact Administrator.' });
      }
      if (!row.role) {
...
router.post('/google-login', async (req, res) => {
  ...
  try {
    ...
    // Note: We bypass the ALLOWED_DOMAIN check here per user request, 
    // relying strictly on the database existence check below.
    ...
    if (!row) {
      return res.status(404).json({ error: 'Your Google account does not have a registered profile in Zuari Catalyst. Contact Administrator.' });
    }
    if (!row.role) {
      return res.status(403).json({ error: 'No role assigned. Contact your administrator.' });
    }
